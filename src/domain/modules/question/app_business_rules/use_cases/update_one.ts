...
import { IQuestionDOM, IQuestionFDOM, QuestionDOM } from "@questions/enterprise_business/entities/question/question_dom";
...
const build = ({ questionsRepo }: Dependencies) => {
    const execute = async(id: number, item: IQuestionDOM) => {
        ...
        if(question[0]) {
            ...
            if (!result) throw new ErrorBadRequest(QUESTION_CANT_BE_UPDATED_ERROR);
